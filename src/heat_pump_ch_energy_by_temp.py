...
if __name__ == "__main__":
    ...
    for day in sorted(day_durations, key=lambda x: x['outside_temp']['mean']):
        # pprint(day)
        print(f"Day: {day['start_time']}, mean outside temp: {day['outside_temp']['mean']:.1f}, mean inside temp: {day['inside_temp']['mean']:.1f}, input energy: {day['energy_ch_in']:.1f}, output energy: {day['energy_ch_out']:.1f}, cop {day['cop_ch']:.2f}")
